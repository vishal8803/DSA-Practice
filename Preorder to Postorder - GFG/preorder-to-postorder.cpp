...
Node* construct(int pre[], int size, int &i, int mx, int mn){
    if(i==size)
        return NULL;
    ...
    if(pre[i] > mn && pre[i] < mx){
        ...
        if(i<size){
            root->left = construct(pre,size,i,root->data,mn);
        }
        
        if(i<size){
            root->right = construct(pre,size,i,mx,root->data);
        }
    }
...
{
    int i = 0;
    return construct(pre,size,i,INT_MAX,INT_MIN);
}
